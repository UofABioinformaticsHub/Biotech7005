...
RStudio has a whole of extra features not diectly related to `R`, but that make our experience much more enjoyable whilst working with `R`.
...
- In recent versions of RStudio we also have a `Terminal` which we have already used to change our password. We'll also spend a fair bit of time here laer in the course.
...
If you'd like to do this, using the RStudio menu bar select `Tools > Global Options > Packages`, then untick the box next to `Enable packages pane`.
</details>  
