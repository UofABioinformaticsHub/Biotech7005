---
title: "Topic 4: Using R Markdown"
...
A 8header section8 is contained between the `---` lines at the top
...
1. Load `pander` into the workspace by placing `library(pander)` below `library(dplyr)`

Then head back to the code chunk and add the following.
Note that we have to remove the pretty `tibble` wrapping paper for this to work, which we do by converting to a generic `data.frame` object.
