...
<!-- We'll also need the softwar `featureCounts` from the `Subread` set of tools, and this can be installed as follows: -->
...
biocLite(c("limma", "voom", "edgeR", "Glimma", "biomaRt"))
```
...
An easy way to do this would be to convert our gene identifiers to EntrezGeneidentifiers, then use the functions `goana()` or `kegga()` in the limma package.
