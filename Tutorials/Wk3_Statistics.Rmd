...
3. The sample mean ($\bar{x}$) is Normally Distributed around the population mean
...
a. The SNP at http://exac.broadinstitute.org/variant/3-8787263-G-A was found in 0.09% of 16366 South Asian genomes, and 0.05% of 8644 East Asian genomes. *What do these numbers represent?*
...
a) We think that a gene will be silenced by overexpression of a targeted siRNA. *What would our null hypothesis and alternate hypothesis be?*
...
This is a histogram of the sample means. Does it look like a normal distribution?
...
We can clearly see that each sample mean is a single random sample from this distribution.

### Q5

In the above, we knew the variance ($\sigma = 1$) because we set this in our random sampling step.
In reality we don't know this, so we use our sample variance ($s^2$) and a $t$-test.
...
We know the true value for the population mean of these random samples ($\mu = 5$).
In reality, we wouldn't know this and we'd need to conduct a hypothesis test with a null hypothesis and alternate hypoethesis.
...
[NB: Here the degrees of freedom is n - 1]

Let's check this against the distribution for $t_9$
...
This will give identical results to the above, but instead each adjusted-$p$ value will be $min(1, p*m)$, where $m = 1000$ represents the number of tests we are performing.
...
rownames(counts) <- c("Normoxia", "Hypoxia")
colnames(counts) <- c("Allele A", "Allele B")
```

```{r, echo=FALSE, results='asis'}
pander::pander(counts, justify = c("lrr"))
```
...
Alternatively, we could've used a $\chi^2$ test, however we would've need to ensure that now cell in the table had an expected count $<5$.
