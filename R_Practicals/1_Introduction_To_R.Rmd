...
First we will setup an `R Project` for today's practical.
...
Like most programming languagesm `R` is very strict about data formats.
...
OSX and Linx use the `/` symbol, so `R` has a convenient function called `file.path()` which is able to decide which separator to put there.
...
That first column looks like it's just row numbers, which are unneccessary in R, as they are automatically included in `data.frame` (and `tibble`) objects.
...
Now we can get it right by telling `read_csv()` to ignore any reows beginning with `#`
