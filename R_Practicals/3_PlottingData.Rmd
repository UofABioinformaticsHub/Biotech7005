...
However, it has a unique syntax and approach where we plot by adding layers of relevant information.
...
Let's leave the general aesthetics in `ggplot()`, with the colour and the shape specific aesthetics in that line.
...
This defaults to a loess fit, so let's select a regression line straight away.
...
We could facet these on transport as well
...
This has a slightly unusual set of default values, we we need to tell it not to summarise anything by setting `stat = "identity"`, and we need to tell it to plot bars next to each other by using `position ="dodge"`.
...
You can also set a default theme for a workspace using `theme_set()`.
This would be placed near the call to `library(ggplot2)` at the start of your script.
