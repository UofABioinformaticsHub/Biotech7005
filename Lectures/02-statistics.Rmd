...
1. Imagine we are examining every human gene (~25,000) for differential expression using RNASeq
...
- If we set the rejection value to $\alpha* = \frac{\alpha}{n}$ we control the FWER at the level $\alpha$
- To ensure that $p$(one or more Type I errors) = 0.05 in our example:<br><br>$\implies$ Reject $H_0$ if $p < \frac{0.05}{25000}$
...
For those interested, the BH procedure for $n$ tests is (not-examinable)

1. Arrange $p$-values in ascending order $p_{(1)}, p_{(2)}, ..., p_{(n)}$ 
2. Find the largest number $k$ such that $p_{(k)} \leq \frac{k}{n}\alpha$
